...
class InstanceCreationPattern {

    // ok: xmlreader-xxe-parameter-entities
    private static XMLReader staticReader;
    ...
    public example0(InputSource is, SAXParser saxParser){

        // ok: xmlreader-xxe-parameter-entities
        XMLReader reader;

        if( saxParser == null ){
            // ruleid: xmlreader-xxe-parameter-entities
    ...
    public example1(InputSource is, SAXParser saxParser){
    ...
    public example2(InputSource is, SAXParser saxParser){
    ...
    public example3(InputSource is, SAXParser saxParser){
    ...
    public example4(InputSource is, SAXParser saxParser){
...
class Foobar {
    ...
    public void doSmth(InputSource is) {
    ...
    public void doSmth2(InputSource is) {
        // ruleid: xmlreader-xxe-parameter-entities
        this.rdr = SAXParserFactory.newInstance().newSAXParser().getXMLReader();
        rdr.parse(is);
    }

    public void doSmthSecure(InputSource is) {
        // ok: xmlreader-xxe-parameter-entities
        var reader = SAXParserFactory.newInstance().newSAXParser().getXMLReader();
        reader.setFeature("http://apache.org/xml/features/disallow-doctype-decl", true);
        reader.parse(is);
    }
}
...
public class Tests {
  ...
  public static void testAccessExternalDTD2() throws SAXException, ParserConfigurationException {
    System.out.println("setAttribute(XMLConstants.ACCESS_EXTERNAL_DTD, \"\")");
    // XMLReader reader = XMLReaderFactory.createXMLReader();
    //ok: xmlreader-xxe-parameter-entities
    XMLReader reader = SAXParserFactory.newInstance().newSAXParser().getXMLReader();
