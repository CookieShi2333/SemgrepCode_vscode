...
response.setHeader("X-XSS-Protection", "0");
        Object[] obj = { "a", bar};
        // ok: no-direct-response-writer-deepsemgrep
...
public class BenchmarkTest02388 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        request = request.getParameter("param");
        // ok: no-direct-response-writer-deepsemgrep
        response.getWriter().println(request);
        // ok: no-direct-response-writer-deepsemgrep
        response.getWriter().printf(request);
        String parameter = request.getParameterx("myParam");

        // ok: no-direct-response-writer-deepsemgrep
        response.getWriter().println(parameter);
        // ok: no-direct-response-writer-deepsemgrep
        response.getWriter().printf(parameter);
